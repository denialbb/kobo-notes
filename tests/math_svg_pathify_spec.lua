...
describe("MathSvgPathify", function()
    ...
    it("rewrites <text> glyphs into font-independent <path> outlines", function()
        local input = [[<svg xmlns="http://www.w3.org/2000/svg" width="100" height="50"><text x="0" y="0" font-family="XITS Math" font-size="1px" transform="matrix(32,0,0,32,100,50)" fill="rgb(0,0,0)" fill-opacity="1" style="white-space: pre;">&#8721;</text></svg>]]
        ...
        assert.is_truthy(out:find("matrix(0.032000,", 1, true), "x-scale maps font units to device px")
        assert.is_truthy(out:find("-0.032000,", 1, true), "y-scale is flipped (SVG y-down)")
    ...
    it("preserves non-text geometry such as <line>", function()
        local input = [[<svg><text x="0" y="0" font-family="XITS Math" font-size="1px" transform="matrix(1,0,0,1,0,0)" fill="rgb(0,0,0)" fill-opacity="1">&#8721;</text><line x1="0" y1="0" x2="10" y2="0" stroke="rgb(0,0,0)" stroke-width="0.04"/></svg>]]
        local out = MathSvgPathify.convert(input)
    ...
    it("leaves unknown codepoints as <text> (graceful degradation)", function()
        local input = [[<svg><text x="0" y="0" font-family="XITS Math" font-size="1px" transform="matrix(1,0,0,1,0,0)" fill="rgb(0,0,0)" fill-opacity="1">&#57344;</text></svg>]] -- U+E000, not in the glyph table
        local out = MathSvgPathify.convert(input)
    ...
    it("leaves multi-codepoint <text> elements unchanged (no silent char loss)", function()
        ...
        local input = [[<svg><text x="0" y="0" font-family="XITS Math" font-size="1px" transform="matrix(1,0,0,1,0,0)" fill="rgb(0,0,0)" fill-opacity="1">&#945;&#768;</text></svg>]]
        local out = MathSvgPathify.convert(input)
    ...
    it("folds non-zero x/y text position into the path translate", function()
        ...
        local input = [[<svg><text x="5" y="7" font-family="XITS Math" font-size="1px" transform="matrix(1,0,0,1,10,20)" fill="rgb(0,0,0)" fill-opacity="1">&#8721;</text></svg>]]
        local out = MathSvgPathify.convert(input)
